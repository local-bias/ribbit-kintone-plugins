/**
 * cybozu共通APIのグローバル型定義
 *
 * 本来は`@konomi-app/kintone-utilities`が同梱していた`cybozu.d.ts`を、各アプリの
 * `tsconfig.json`の`files`から参照していました。しかし同パッケージのv6(tsupへの移行時に
 * `package.json`へ`files: ["dist"]`が追加された)以降、`cybozu.d.ts`が公開物に含まれなく
 * なったため、参照が解決できずtscがTS6053で即座に失敗する状態になっていました。
 *
 * `cybozu`グローバル名前空間を参照するアプリ (uid / smart-view / tooltip) で共有するため、
 * 必要な定義をこの共有パッケージへ暫定的に配置しています。参照する側は
 * `tsconfig.json`の`files`に`./node_modules/@repo/typescript-config/cybozu.d.ts`を追加してください。
 * `@konomi-app/kintone-utilities`側が`cybozu.d.ts`を再び同梱するようになったら、
 * このファイルを削除して同パッケージを参照する形へ戻してください。
 *
 * 内容は`@konomi-app/kintone-utilities` v6.3.0のリポジトリにある`packages/core/cybozu.d.ts`
 * と同一です。
 */
