...
export const isUsagePluginConditionMet = (condition: PluginCondition) => {
  ...
  // 少なくとも1つの表示画面が選択されている必要がある
  const hasDisplayScreen =
    condition.displayScreens.index ||
    condition.displayScreens.show ||
    condition.displayScreens.edit;
  if (!hasDisplayScreen) {
    return false;
  }

  // ページモードに応じた必須項目のチェック
  if (condition.pageMode === 'app' && !condition.appId) {
    return false;
  }
  if (condition.pageMode === 'url' && !condition.url) {
    return false;
  }
