...
export const filteredTableRowsAtom = atom<TableRow[]>((get) => {
  ...
  const lastFiltered = firstFiltered.filter(({ record }) =>
    extractedSearchConditions.slice(0, 1).every(({ type, value, fieldCode }) => {
      ...
      switch (type) {
        case 'text': {
          const input = getYuruChara(value as string, yuruCharaOptions);
        ...
        case 'autocomplete': {
          return getFieldValueAsString(field) === (value as string);
        }
        case 'multi-select': {
        ...
        case 'date': {
          return ~getFieldValueAsString(field).indexOf(value as string);
        }
        case 'month': {
          const month = (value as string).slice(0, 7);
          return ~getFieldValueAsString(field).indexOf(month);
        }
        case 'year': {
          const year = (value as string).slice(0, 4);
