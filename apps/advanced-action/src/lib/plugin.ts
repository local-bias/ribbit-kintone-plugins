...
export const isUsagePluginConditionMet = (condition: PluginCondition) => {
  return LatestPluginConditionSchema.safeParse(condition).success && condition.fields.length > 0;
};

export function getNewAppConnection(): PluginCondition['appConnections'][number] {
  return {
    id: nanoid(),
    appId: '',
    srcFieldCode: '',
...
export function getNewField(): PluginCondition['fields'][number] {
  return {
    id: nanoid(),
    type: 'copy',
    srcAppId: '',
...
export const getNewCondition = (): PluginCondition => ({
  ...
  mode: 'source',
  dstAppId: '',
  buttonLabel: 'レコードをコピー',
  appConnections: [getNewAppConnection()],
  fields: [getNewField()],
  users: [],
});
